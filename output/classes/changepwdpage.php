...
class ChangePasswordPage extends RunnerPage
{
	...
	protected function getSQLWhere()
	{
		...
		if( $this->token )
			return " where".$this->connection->addFieldWrappers( "reset_token2" )."=". $this->connection->prepareString( $this->token );
	...
	protected function getUpdateSQL( $newpass, $where )
	{
		...
		if( $this->token )
		{
			$setPart.= ", ".$this->connection->addFieldWrappers( "reset_token2" )."=". $this->connection->prepareString( "" ).", "
	...
	protected function checkToken()
	{
		...
		$sqlSelect = "select ".$this->connection->addFieldWrappers( "reset_date" )." from".$this->connection->addTableWrappers("tb_usuarios")
			." where".$this->connection->addFieldWrappers( "reset_token2" )."=". $this->connection->prepareString( $this->token );
