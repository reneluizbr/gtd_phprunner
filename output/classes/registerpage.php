...
class RegisterPage extends RunnerPage
{
	...
	protected function registerNewUser()
	{
		...
		$passwordHash = md5( $values["usua_senha"] );
		$originalpassword = $values["usua_senha"];
		//	encrypt password
		if( !$this->cipherer->isFieldEncrypted( $this->passwordFiled ) )
		{
			$passwordHash = $this->getPasswordHash( $values["usua_senha"] );
			$values["usua_senha"] = $passwordHash;
		}

		$retval = DoInsertRecord("tb_usuarios", $values, $blobfields, $this);
		
		$values["usua_senha"] = $originalpassword;
	...
	{
		if( $this->cipherer->isFieldEncrypted("usua_username") )
			$sUsername = $this->cipherer->MakeDBValue("usua_username", $strUsername, "", true);	
		else 
			$sUsername = add_db_quotes("usua_username", $strUsername);
			
		$strSQL = "select count(*) from ". $this->connection->addTableWrappers( "tb_usuarios" ) 
			. " where " . 
			$this->connection->comparisonSQL( 
				$this->getFieldSQLDecrypt("usua_username"), 
				$sUsername,
