...
class RemindPasswordPage extends RunnerPage
{
	...
	{		
		...
		if( $this->remindMethod() == RPM_RESET )
		{
			...
			$this->connection->exec("update ".$this->connection->addTableWrappers("tb_usuarios")." set "
				.$this->connection->addFieldWrappers( "reset_token2" )."=". $this->connection->prepareString( $token ).","
		...
		else
		{
				if( !$this->cipherer->isFieldEncrypted( $this->passwordFiled ) )
			{
				...
				$this->connection->exec("update ".$this->connection->addTableWrappers("tb_usuarios")
 ...
 //RPM_RESET
	...
	protected function getUserSearchingSQLData()
	{
		...
		{
			$fullcUserNameField = $this->connection->addTableWrappers( "tb_usuarios" ).".".$this->connection->addFieldWrappers( $this->usernameFiled );
			$fullcEmailField = $this->connection->addTableWrappers( "tb_usuarios" ).".".$this->connection->addFieldWrappers( $this->emailField );
			$fullcPasswordField = $this->connection->addTableWrappers( "tb_usuarios" ).".".$this->connection->addFieldWrappers( $this->passwordFiled );
		}
		...
		$strSQLData["sql"] = $selectClause." from ".$this->connection->addTableWrappers("tb_usuarios")." where ".$sWhere;
