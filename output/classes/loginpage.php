...
class LoginPage extends RunnerPage
{
	...
	protected function getSelectSQL( $skipPasswordCheck, $strUsername, $strPassword, $loginSet, $cipherer )
	{
		...
		return "select * from ".$this->connection->addTableWrappers("tb_usuarios")
	...
	public function LoginWithSP( $plugin, $token, $addNewUser = true )
	{
		...
		$strSQL = "select * from ".$this->connection->addTableWrappers( "tb_usuarios" )
			." where "
		...
		else if( $addNewUser )
		{
			...
			$blobfields = array();
			$retval = DoInsertRecord( "tb_usuarios", $data, $blobfields, $this );
			if( !$retval )
				return false;
